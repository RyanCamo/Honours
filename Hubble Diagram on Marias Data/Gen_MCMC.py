...
def metropolis_hastings(data_x, data_y, data_err, begin, nsamples, proposal_width, model):
    ...
    for i in range(1,nsamples):
        ...
        if i%10000 == 0:
            ...
            if int(100.0*acceptance/i) < 20:
                proposalTemp = []
                for i, proposal in enumerate(proposal_width):
                    proposalTemp.append(proposal*0.8)
                    proposal_width = proposalTemp
            if int(100.0*acceptance/i) > 30:
                proposalTemp = []
                for i, proposal in enumerate(proposal_width):
                    proposalTemp.append(proposal*1.2)
                    proposal_width = proposalTemp
...
def get_params(samples, label, model):
    c = ChainConsumer()
    burnin = 1
