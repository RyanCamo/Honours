...
name = IDE1.__name__
# Import Chains #
IDE1_CHAIN = np.loadtxt('/Users/RyanCamo/Documents/GitHub/Honours/Hubble Diagram/Chains_OUTPUT/Chains_OUTPUT_Geta/%s_CHAIN_covloglike.txt' % name)
print(IDE1_CHAIN[20:])
...
c.add_chain(IDE1_CHAIN, parameters=label, linewidth=2.0, name=name, kde=1.5, color="blue",num_free_params=len(begin)).configure(summary=True, shade_alpha=0.2,statistics="max")
...
c.plotter.plot_contour(ax, r"$\omega$", r"$\epsilon$") #r"$\Omega_{CDM}$", r"$\Omega_{DE}$", r"$\omega$", r"$\epsilon$"
