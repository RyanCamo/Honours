...
oz = c.analysis.get_summary(chains=name)['$w_z$'][1]
ozp =c.analysis.get_summary(chains=name)['$w_z$'][2]-c.analysis.get_summary(chains=name)['$w_z$'][1]
ozm =c.analysis.get_summary(chains=name)['$w_z$'][1]-c.analysis.get_summary(chains=name)['$w_z$'][0]
...
ax.text(-0.15,-2-0.76,'$\omega_{z} = %10.5s^{%10.5s}_{%10.5s}$' %(oz,ozp,ozm), family='serif',color='black',rotation=0,fontsize=12,ha='right')
